"""
Script for extracting commit features that characterize the experience of developers.
"""
...
from core.config import repository_path_location, repository_branch, project_name, by_tag
    ...
    for i, commit in enumerate(tqdm(commits[1:])):
        ...
        if type == 1:
            ...
            if author not in all_authors:
                ...
                if overall <= 0:
                    overall = 1
                all_authors[author][commit.hex]["sexp"] = float(
                    modified_systems / ((overall + diffing_years) + overall))
            if i % 10 == 0:
                gc.collect()
